...
pub fn parse(tokens: &[Token]) -> Result<Program, String> {
    ...
    while !p.at(&Token::Eof) {
        ...
        if p.at(&Token::Function) { functions.push(p.parse_function()?); }
        else { main.push(p.statement(false)?); }
        p.skip_newlines();
    }
...
impl<'a> Parser<'a> {
    ...
    fn statement(&mut self, in_function: bool) -> Result<Stmt, String> {
        match self.tokens.get(self.pos) {
            ...
            Some(Token::Identifier(_)) => { let name = self.identifier()?; if !self.at(&Token::Equal) { return Err("expected `=` after variable name".into()); } self.pos += 1; Ok(Stmt::Assign(name, self.expr()?)) }
            ...
            _ => Err("expected `let`, `print`, `if`, `while`, or `return`".into()),
        }
    }
    ...
    fn parse_function(&mut self) -> Result<FunctionDecl, String> {
        ...
        if !self.at(&Token::LeftParen) { return Err("expected `(` after function name".into()); }
        self.pos += 1;
        let mut params = Vec::new();
        if !self.at(&Token::RightParen) {
            loop {
                let param = self.identifier()?;
                if params.contains(&param) { return Err(format!("duplicate parameter `{param}`")); }
                params.push(param);
                if self.at(&Token::Comma) { self.pos += 1; } else { break; }
            }
        }
        if !self.at(&Token::RightParen) { return Err("expected `)` after function parameters".into()); }
        self.pos += 1;
        self.skip_newlines();
        let body = self.block(true, false)?;
        if !self.at(&Token::End) { return Err(format!("expected `end` after function `{name}`")); }
        self.pos += 1;
    ...
    fn block(&mut self, stop_end: bool, stop_else: bool) -> Result<Vec<Stmt>, String> {
        ...
        loop {
            ...
            if self.at(&Token::Function) { return Err("nested functions are not supported".into()); }
            out.push(self.statement(true)?);
            self.skip_newlines();
        }
        Ok(out)
    }
    ...
    fn parse_if(&mut self, in_function: bool) -> Result<Stmt, String> {
        ...
        let else_body = if self.at(&Token::Else) { self.pos += 1; self.skip_newlines(); self.block(true, true)? } else { Vec::new() };
    ...
    fn parse_while(&mut self, in_function: bool) -> Result<Stmt, String> {
        self.pos += 1;
        let condition = self.expr()?;
        if self.at(&Token::Then) { self.pos += 1; }
    ...
    fn identifier(&mut self) -> Result<String, String> {
        match self.tokens.get(self.pos) { Some(Token::Identifier(s)) => { let n=s.clone(); self.pos += 1; Ok(n) }, _ => Err("expected identifier".into()) }
    }
    fn expect(&mut self, token: Token, message: &str) { if self.at(&token) { self.pos += 1; } }
    ...
    fn expr(&mut self) -> Result<Expr, String> {
        ...
        match self.tokens.get(self.pos) {
            Some(Token::EqualEqual)=>self.compare(left,CompareOp::Eq), Some(Token::NotEqual)=>self.compare(left,CompareOp::Ne),
            Some(Token::Less)=>self.compare(left,CompareOp::Lt), Some(Token::LessEqual)=>self.compare(left,CompareOp::Le),
            Some(Token::Greater)=>self.compare(left,CompareOp::Gt), Some(Token::GreaterEqual)=>self.compare(left,CompareOp::Ge), _=>Ok(left)
        }
    }
    ...
    fn term(&mut self) -> Result<Expr,String> {
        ...
        loop { let op=match self.tokens.get(self.pos) { Some(Token::Star)=>Op::Mul, Some(Token::Slash)=>Op::Div, _=>break }; self.pos+=1; left=Expr::Binary(Box::new(left),op,Box::new(self.primary()?)); }
        Ok(left)
    }
    fn primary(&mut self) -> Result<Expr,String> {
        match self.tokens.get(self.pos) {
            ...
            Some(Token::Identifier(s))=>{
                let name=s.clone(); self.pos+=1;
                if self.at(&Token::LeftParen) {
                    self.pos+=1; let mut args=Vec::new();
                    if !self.at(&Token::RightParen) { loop { args.push(self.expr()?); if self.at(&Token::Comma) { self.pos+=1; } else { break; } } }
                    if !self.at(&Token::RightParen) { return Err("expected `)` after function arguments".into()); }
                    self.pos+=1; Ok(Expr::Call(name,args))
