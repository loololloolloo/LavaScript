...
fn collect_locals_into(statements: &[Stmt], locals: &mut HashMap<String, u32>, consts: &mut HashSet<String>) -> Result<(), String> {
    for stmt in statements {
        match stmt {
            Stmt::Let(name, _) => { if locals.contains_key(name) { return Err(format!("variable '{}' is already declared", name)); } let index = locals.len() as u32; locals.insert(name.clone(), index); }
            Stmt::Const(name, _) => { if locals.contains_key(name) { return Err(format!("variable '{}' is already declared", name)); } let index = locals.len() as u32; locals.insert(name.clone(), index); consts.insert(name.clone()); }
...
fn emit_expr(function: &mut Function, expr: &Expr, vars: &HashMap<String, u32>, signatures: &Signatures, strings: &mut StringTable) -> Result<(), String> {
    match expr {
        ...
        Expr::Call(name, args) => { let &(index, arity) = signatures.get(name).ok_or_else(|| format!("undefined function '{}'", name))?; if args.len() != arity { return Err(format!("function '{}' expects {} argument(s), got {}", name, arity, args.len())); } for arg in args { emit_expr(function, arg, vars, signatures, strings)?; } function.instruction(&Instruction::Call(index)); }
    }
    Ok(())
}
