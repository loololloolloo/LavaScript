...
fn compile_program(program: &Program) -> Result<Vec<u8>, String> {
    ...
    for (i, function) in program.functions.iter().enumerate() {
        if signatures.insert(function.name.clone(), (2 + i as u32, function.params.len())).is_some() {
    ...
    emit_statements(&mut main, &program.main, &main_locals, &main_consts, &signatures, false, &mut strings, 0, 0)?;
...
fn emit_statements(function:&mut Function,statements:&[Stmt],vars:&HashMap<String,u32>,consts:&HashSet<String>,signatures:&Signatures,in_function:bool,strings:&mut StringTable,loop_depth:usize,continue_depth:usize)->Result<(),String>{
    for stmt in statements{match stmt{
        ...
        Stmt::For{name,start,end,step,body}=>{let step_value=match step{Expr::Number(n) if *n!=0=>*n,_=>return Err("`for` step must be a non-zero numeric literal".into())};let index=*vars.get(name).ok_or_else(||format!("undefined variable `{name}`))?;emit_expr(function,start,vars,signatures,strings)?;function.instruction(&Instruction::LocalSet(index));function.instruction(&Instruction::Block(BlockType::Empty));function.instruction(&Instruction::Loop(BlockType::Empty));function.instruction(&Instruction::LocalGet(index));emit_expr(function,end,vars,signatures,strings)?;if step_value>0{function.instruction(&Instruction::I32GtS);function.instruction(&Instruction::I32Eqz);}else{function.instruction(&Instruction::I32LtS);function.instruction(&Instruction::I32Eqz);}function.instruction(&Instruction::BrIf(1));emit_statements(function,body,vars,consts,signatures,in_function,strings,loop_depth+1,0)?;function.instruction(&Instruction::LocalGet(index));function.instruction(&Instruction::I32Const(step_value));function.instruction(&Instruction::I32Add);function.instruction(&Instruction::LocalSet(index));function.instruction(&Instruction::Br(0));function.instruction(&Instruction::End);function.instruction(&Instruction::End);}
...
#[test]fn rejects_duplicate_functions(){assert!(compile("function x(a)\nreturn a\nend\nfunction x(b)\nreturn b\nend").is_err());}
}