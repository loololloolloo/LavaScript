...
pub fn compile(source: &str) -> Result<Vec<u8>, String> {
    ...
    let local_declarations = if locals.is_empty() { Vec::new() } else { vec![(locals.len() as u32, ValType::I32)] };
...
fn collect_locals(statements: &[Stmt], locals: &mut HashMap<String, u32>) -> Result<(), String> {
    for stmt in statements {
        match stmt {
            ...
            Stmt::Assign(name, _) => {
                if !locals.contains_key(name) { return Err(format!("undefined variable `{name}`")); }
            }
...
fn emit_statements(function: &mut Function, statements: &[Stmt], vars: &HashMap<String, u32>) -> Result<(), String> {
    for stmt in statements {
        match stmt {
            ...
            Stmt::Print(expr) => { emit_expr(function, expr, vars)?; function.instruction(&Instruction::Call(0)); }
            Stmt::If { condition, then_body, else_body } => {
                emit_expr(function, condition, vars)?;
                function.instruction(&Instruction::If(BlockType::Empty));
                emit_statements(function, then_body, vars)?;
                if !else_body.is_empty() { function.instruction(&Instruction::Else); emit_statements(function, else_body, vars)?; }
...
fn emit_expr(function: &mut Function, expr: &Expr, vars: &HashMap<String, u32>) -> Result<(), String> {
    match expr {
        ...
        Expr::Binary(left, op, right) => {
            emit_expr(function, left, vars)?; emit_expr(function, right, vars)?;
