...
pub enum Token {
    ...
    AndAnd, OrOr, Bang,
    LeftParen, RightParen, LeftBracket, RightBracket, LeftBrace, RightBrace, Comma, Colon, Dot, Semicolon, Newline, Eof,
}
...
pub fn lex(source: &str) -> Result<Vec<Token>, String> {
    ...
    while i < chars.len() {
        match chars[i] {
            ...
            '&' => { i += 1; if i + 1 < chars.len() && chars[i + 1] == '&' { tokens.push(Token::AndAnd); i += 2; } else { return Err("expected `&&`".into()); } }
            '|' => { i += 1; if i + 1 < chars.len() && chars[i + 1] == '|' { tokens.push(Token::OrOr); i += 2; } else { return Err("expected `||`".into()); } }
            '<' => { i += 1; if i < chars.len() && chars[i] == '=' { tokens.push(Token::LessEqual); i += 1; } else { tokens.push(Token::Less); i += 1; } }
            '>' => { i += 1; if i < chars.len() && chars[i] == '=' { tokens.push(Token::GreaterEqual); i += 1; } else { tokens.push(Token::Greater); i += 1; } }
...
#[test]fn lexes_collections(){assert_eq!(lex("[1, 2] {x: 3}.x").unwrap(),vec![Token::LeftBracket,Token::Number(1),Token::Comma,Token::Number(2),Token::RightBracket,Token::LeftBrace,Token::Identifier("x".into()),Token::Colon,Token::Number(3),Token::RightBrace,Token::Dot,Token::Identifier("x".into()),Token::Eof]);}}
